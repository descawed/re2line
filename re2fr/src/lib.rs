...
impl FlightRecorder {
    pub fn apply_patches(&mut self) -> Result<()> {
        ...
        unsafe {
            ...
            let frame_tick_thunk = self.frame_tick.bind(frame_hook_mov_address, frame_tick as *const ())?;

            log::info!("Installing RNG tracker hook at {:08X}", version.rng_roll_patch);
            // patched instruction is 6 bytes so we need to pad with nops
            Hook::call(version.rng_roll_patch as *mut c_void, rng_track_thunk).pad_to(6).install_persistent()?;

            log::info!("Installing frame tick hook at {:08X}", version.frame_tick_patch);
            Hook::call(version.frame_tick_patch as *mut c_void, frame_tick_thunk).install_persistent()?;
        }
